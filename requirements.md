...
def calculate_derived_metrics(df, initial_investment, periods_per_year):
    ...
    # 計算年化報酬率
    enhanced_df["Annualized_Return"] = enhanced_df.apply(
        lambda row: calculate_annualized_return(
            row["Cum_Value"], 
            row["Cum_Inv"], 
            (row["Period"] + 1) / periods_per_year
        ) if row["Period"] > 0 else 0, 
        axis=1
    )
...
STRATEGY_COMPARISON_CARDS = {
    ...
    "va_strategy_card": {
        ...
        "calculation_backend": {
            ...
            "return_calculation": "calculate_annualized_return",
...
from src.performance_metrics import calculate_annualized_return, calculate_irr
...
1. **簡化初始化**：移除複雜的系統監控，保留基本API檢查
2. **基本計算流程**：使用Streamlit內建的spinner進度提示
3. **直接錯誤恢復**：簡單的fallback機制
4. **Streamlit Cloud部署**：專注於最簡單的部署方式
5. **核心功能保留**：VA/DCA計算、基本圖表、CSV下載
